...
class HiddenMarkovModel:
    ...
    # Function to generate first of two rhyming lines
    def generate_original_line(self, syll_dict, rhyme_dict, obs_map, obs_map_r, max_syll=10):
        '''
        Generates an emission of length M, assuming that the starting state
        is chosen uniformly at random.
        ...
        # Force first word of sequence (last word of line) to have a rhyming partner
        while(True):
            state = random.choice(range(self.L))
            rand_var = random.uniform(0, 1)
            next_obs = 0
            ...
            next_obs -= 1
            next_word = obs_map_r[next_obs]
            if next_word in rhyme_dict:
                states.append(state)
                emission.append(next_obs)
                last_word = next_obs
                # Get syllable count of the word corresponding to the index of the observation
                syll_count += syll_dict[next_word]
                break
        ...
        while True:
            ...
            syllables = syll_dict[obs_map_r[next_obs]]
            if syll_count + syllables == max_syll:
                emission.append(next_obs)
                states.append(state)
                break
            elif syll_count + syllables > max_syll:
                continue
            else:
                syll_count += syllables
                emission.append(next_obs)
                states.append(state)
        ...
        return emission, states, last_word

    def generate_rhyming_line(self, obs, syll_dict, rhyme_dict, obs_map, obs_map_r, max_syll=10):
        emission = []
        states = []
        ...
        # Force first word of sequence (last word of line) to be obs
        emission.append(obs)
        syll_count += syll_dict[obs_map_r[obs]]
